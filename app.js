...
// This adapter is deliberately explicit: confirm aliases against the real files.
const LAYERS = [
...
function details(properties) {
  ...
  if (url) { const link = element('a', 'Vezi obiectul OpenStreetMap ↗'); link.href = url; link.target = '_blank'; link.rel = 'noopener noreferrer'; box.append(link); }
  return box;
}
function validateGeoJSON(data, expected) {
  ...
  for (const [i, feature] of data.features.entries()) {
    if (feature.type !== 'Feature' || !geometry(feature.geometry)) throw new Error(`Geometrie invalidă la înregistrarea ${i + 1}.`);
    if (!feature.properties || typeof feature.properties !== 'object' || Array.isArray(feature.properties) || layerIndex(feature.properties) < 0) throw new Error(`Strat necunoscut la înregistrarea ${i + 1}. Verificați adaptorul FIELDS / LAYERS.`);
  }
  return data;
}
...
function parseCSV(text) {
  ...
  if (!headers.length || headers.some(h => !h) || new Set(headers).size !== headers.length) throw new Error('Antet CSV invalid.');
...
async function fetchText(url) {
  ...
  if (!response.ok) throw new Error(response.status === 404 ? 'Fișierul real nu este încă disponibil. Va fi încărcat separat înainte de publicarea finală.' : `Eroare HTTP ${response.status}.`);
...
async function start() {
  ...
  const configs = {full: {link: $('download-full'), expected: 142}, strict: {link: $('download-strict'), expected: 136}};
  for (const config of Object.values(configs)) config.link.addEventListener('click', event => { if (config.link.getAttribute('aria-disabled') === 'true') event.preventDefault(); });
  let csvRecords;
  $('no-geometry').addEventListener('click', async () => {
    $('missing-dialog').showModal();
    const content = $('missing-content'); content.textContent = 'Se încarcă…';
    try {
      if (!csvRecords) {
        const rows = parseCSV(await fetchText($('no-geometry').dataset.source));
        if (rows.length !== 4) throw new Error(`Sunt așteptate 4 cazuri; CSV-ul conține ${rows.length}.`);
        csvRecords = rows;
      }
      content.replaceChildren();
      for (const record of csvRecords) {
        const article = element('article', undefined, 'record'); article.append(details(record));
        const extra = element('dl');
        // Preserve every CSV column, even before its schema has been confirmed.
        for (const [key, value] of Object.entries(record)) extra.append(element('dt', key), element('dd', value || '—'));
        const raw = element('details'); raw.append(element('summary', 'Toate câmpurile originale'), extra); article.append(raw); content.append(article);
      }
    } catch (error) { content.textContent = error.message; }
  });
  const selected = new Set(LAYERS.map((_, i) => i));
  LAYERS.forEach((layer, i) => {
    const label = element('label'), input = element('input'), swatch = element('span', '', 'swatch');
    input.type = 'checkbox'; input.checked = true; swatch.style.backgroundColor = layer.color;
    label.append(input, swatch, document.createTextNode(layer.name)); $('filters').append(label);
  ...
  function bounds(layer) { return layer.getBounds ? layer.getBounds() : L.latLngBounds([layer.getLatLng()]); }
  function highlight(record) {
    document.querySelectorAll('#results button').forEach(button => button.setAttribute('aria-current', String(button.dataset.record === String(record.index))));
  }
  function renderList() {
    const inView = $('in-view').checked;
    const listed = visible.filter(record => !inView || map.getBounds().intersects(bounds(record.layer)));
    $('results').replaceChildren();
    $('count').textContent = `${listed.length} în listă · ${visible.length} rezultate filtrate · ${records.length} încărcate`;
    ...
    for (const record of listed) {
      const li = element('li'), button = element('button'), swatch = element('span', '', 'swatch');
      button.type = 'button'; button.dataset.record = record.index; swatch.style.backgroundColor = LAYERS[record.category].color;
      button.append(swatch, document.createTextNode(` ${record.name}`), element('small', `${field(record.properties, 'id') || 'ID nespecificat'} · ${LAYERS[record.category].name}`));
      button.addEventListener('click', () => {
        map.fitBounds(bounds(record.layer), {maxZoom: 14, padding: [35, 35], animate: false});
        record.layer.openPopup(bounds(record.layer).getCenter()); highlight(record);
  ...
  function render() {
    if (!map) return;
    map.closePopup(); group.clearLayers(); const query = normalize($('search').value);
    visible = records.filter(record => selected.has(record.category) && record.search.includes(query));
    visible.forEach(record => group.addLayer(record.layer)); renderList();
  }
  function fit() { if (group.getLayers().length) map.fitBounds(group.getBounds(), {padding: [30, 30], maxZoom: 13}); }
  async function loadMode() {
    const request = ++sequence; records = []; render(); setStatus('Se verifică setul selectat…');
    const current = mode, config = configs[current];
    try {
      const data = cache[current] || validateGeoJSON(JSON.parse(await fetchText(config.link.getAttribute('href'))), config.expected);
      cache[current] = data; config.link.setAttribute('aria-disabled', 'false');
      if (request !== sequence) return;
      records = data.features.map((feature, index) => {
        const properties = feature.properties, category = layerIndex(properties), color = LAYERS[category].color;
        const name = field(properties, 'name') || field(properties, 'id') || 'Nume nespecificat';
        const layer = L.geoJSON(feature, {style: {color, weight: 3, fillOpacity: .2}, pointToLayer: (_, latlng) => L.circleMarker(latlng, {radius: 7, color: '#fff', weight: 2, fillColor: color, fillOpacity: .9})});
        const record = {index, properties, category, name, layer, search: normalize(`${name} ${field(properties, 'id')}`)};
        layer.bindPopup(() => details(properties)); layer.on('popupopen', () => highlight(record));
        return record;
      });
      render(); fit(); setStatus(`${current === 'full' ? 'Complet' : 'Strict'}: ${records.length} geometrii încărcate.`, true);
    } catch (error) { if (request === sequence) setStatus(`Set ${current === 'full' ? 'complet' : 'strict'}: ${error.message}`); }
  }
  $('search').addEventListener('input', render); $('in-view').addEventListener('change', renderList);
  $('fit').addEventListener('click', fit); map.on('moveend', renderList);
  document.querySelectorAll('[name=mode]').forEach(input => input.addEventListener('change', () => { mode = input.value; loadMode(); }));
  await loadMode();
}
if (typeof document !== 'undefined') start().catch(error => { document.getElementById('status').textContent = `Interfața nu a putut porni: ${error.message}`; });
if (typeof module !== 'undefined') module.exports = {parseCSV, validateGeoJSON, layerIndex, osmURL, normalize};
